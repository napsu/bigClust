...
!*     Modules included: 
!*
!*     slmb_mod       ! Stochastic Limited Memory Bundle Method
!*
MODULE slmb_mod  ! Stochastic Limited memory bundle method

    USE r_precision, ONLY : prec  ! Precision for reals.
    IMPLICIT NONE

    ! MODULE slmb_mod includes the following subroutines (S) and functions (F).
    PUBLIC :: &
        slmb_init     ! S Initializing SLMB subroutine for clustering.
    PRIVATE :: &
        slmb, &       ! S Limited memory bundle subroutine for nonsmooth
...
CONTAINS
    ...
    SUBROUTINE slmb_init(x0,x1,fvalue,ns2time)
 
        USE initclust, ONLY : &
            maxdim, &      ! Maximum number of variables
            nrecord => nrec_used, &     ! Number of records
        ...
        USE initslmb, ONLY : & 
            ...
            init_slmbpar   ! S  Further initialization of parameters
        USE exe_time, ONLY : getime   ! Execution time.

        REAL :: start,fini,ns2time 
        REAL(KIND=prec) ::  &
            x0(nc*nft),     &
            x1(nc*nft),     &
            fvalue
        ...
        INTEGER, DIMENSION(4) :: &
            iout           ! Output integer parameters.
                           ...
                           !               6  - Time limit exceeded. hihu tämä on nyt klusterointi algoritmin puolella
                           !               7  - f < tolb.
                           !               8  - Maximum number of batches computed
        ...
        IF (nbatch > nrecord) PRINT*,'nbatch is greater than nrecords.'

        ! CPU-time
        CALL getime(start)

        CALL slmb(mc,fvalue,iout(1),iout(2),iout(3),iout(4))
    ...
    !*     * SUBROUTINE slmb *                                             *
    !*                                                                     *
    !*     Stochastic limited memory bundle subroutine for                 * 
    !*     nonsmooth optimization.                                         *
    !*                                                                     *
    !***********************************************************************
      
    SUBROUTINE slmb(mc,f,nit,nfe,nge,iterm)
        ...
        USE initslmb, ONLY : &
            ...
            batchind, &  ! Table of batch indices  
            ib, &        ! The first batch index
        ...
        USE initclust, ONLY : &
            nrecord => nrec_used, & ! Number of used records in data.
            ns           ! Switch for auxiliary and cluster problems
        USE obj_fun, ONLY : &
            myf, &       ! Computation of the value of the objective function.
            myg, &       ! Computation of the subgradient of the objective function.
            clustfull    ! Computation of full cluster function value (with partial data if nrecord /= nrec_used)
        ...
        ! Local Scalars
        REAL(KIND=prec) :: &
            fbest, &     ! Best obtained objective value.
        ...
        integer :: j,itmp,ibatch,fcount,inth
        ...
        batch: DO ibatch = 1,maxbatch
            ...
            iteration: DO
                ...
                ELSE
                    ...
                    IF (nres == 1) THEN
                        ...
                        CALL restar(n,mc,mcc,mcinit,inew,ibun,ibfgs,iters,gp, &
                            g,nnk,alfv,alfn,gamma,d,ic,icn,mal,ncres,iflag)
                ...
                IF (f <= tolb) THEN
                    iterm = 7
                    EXIT iteration ! Error in clustering
                END IF
                ...
                IF (iters == 0) THEN
                    IF (ABS(xnorm - pxnorm) <= small) THEN
                        ...
                        IF (neps > maxeps) THEN
                            iterm = -4
                            EXIT iteration ! iterm < 0 clean up the memory and go to next batch
                        END IF
                ...
                if (mod(nit, nth)==0 .or. inth == 1) THEN ! compute next batch
                    IF (iters > 0) THEN
                        EXIT iteration
                    ELSE
                        inth = 1
            ...
            if (iterm == 7) PRINT*,' Something is wrong, iterm = 7, f=',f ! Note, this never happened in tests.
        
            ! SLMB: Randomly select a new batch
            if (nrecord /= nbatch) then
                if (batchtype == 1) then
                    ! batches such that all indices are used at least once
                    ib = ib+nbatch
                    if (ib > nrecord-nbatch) then
                        ib = 0 
                    end if    
                end if
            ...
            ncres  = -1 ! ncres is number of restarts in current batch
            ...
            if (iterm >= 0) then    
            ...
            else if (iterm == -1 .or. iterm == -2 .or. iterm == -4) then
                ...
                print*,'Fatal failure.'
                EXIT batch ! Fatal failure, stop computing.
            end if
